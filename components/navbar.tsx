      ...
      <nav
        ...
        <div className="container mx-auto px-6 lg:px-12">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <div ref={logoRef} className="relative z-10 h-12">
              <Link href="/" className="group flex items-center h-full">
                {/* Desktop Logo */}
                <img
                  src="/navbar_logo.png"
                  alt="ALDEE'S Logo"
                  className="hidden sm:block h-20 md:h-24 w-auto object-contain group-hover:scale-105 transition-transform duration-300"
                />
                {/* Mobile Logo */}
                <img
                  src="/navbar_mobile_logo.png"
                  alt="ALDEE'S"
                  className="block sm:hidden h-16 w-auto object-contain group-hover:scale-105 transition-transform duration-300"
                />
                {/* Glow effect on hover */}
                <div className="absolute inset-0 bg-aldees-yellow/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              </Link>
            </div>
            ...
            <div ref={linksRef} className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  {/* Animated underline */}
                  <span
                    className={`nav-underline absolute bottom-2 left-6 right-6 h-0.5 bg-linear-to-r from-aldees-yellow via-aldees-yellow to-transparent origin-left ${
      ...
      <div
        ...
        <div className="flex flex-col items-center justify-center h-full px-8">
          ...
          <nav className="flex flex-col items-center gap-8 mb-12">
            {navLinks.map((link, i) => (
              <Link
                ...
                />
                {/* Number */}
                <span className="absolute -left-8 top-0 text-sm text-aldees-yellow/40">
                  0{i + 1}
                </span>
              </Link>
            ))}
          ...
          <div className="mt-16 text-center">
            <p className="text-aldees-offwhite/40 text-sm tracking-[0.2em] uppercase mb-2">
              Call Us
            </p>
            <a
              href="tel:+919915824156"
              className="text-2xl text-aldees-yellow font-bold">
              +91 99158 24156
            </a>
          </div>
        </div>
